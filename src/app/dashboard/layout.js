...
import { Menu, X, LogOut, LayoutDashboard, BookOpen, Library, Briefcase, Bell, User as UserIcon, Activity } from "lucide-react";
...
export default function DashboardLayout({ children }) {
  ...
  return (
    <div className="min-h-screen bg-[#f1f5f9] text-slate-900 selection:bg-indigo-100 selection:text-indigo-700">
      ...
      <header 
        className={`sticky top-0 z-50 transition-all duration-300 ${
          scrolled 
            ? "bg-white/80 backdrop-blur-lg border-b border-indigo-100 shadow-sm py-3" 
            : "bg-transparent py-5"
        }`}
      >
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex justify-between items-center text-sm">
          <div className="flex items-center gap-10">
            <Link href="/dashboard" className="flex flex-col group">
              ...
              <div className="flex items-center gap-1.5 md:hidden">
                ...
                <span className="text-[10px] font-bold text-slate-500 tracking-wider uppercase">Live Now</span>
              </div>
            </Link>
          ...
          <div className="hidden lg:flex items-center gap-6">
            <div className="flex flex-col text-right">
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-1">Authenticated as</span>
              <p className="font-bold text-slate-700">{user?.username || "Fetching..."}</p>
            </div>
            <div className="h-8 w-[1px] bg-slate-200" />
            <button
              onClick={handleLogout}
              className="group flex items-center gap-2 bg-white text-slate-600 hover:text-red-600 font-bold px-4 py-2 rounded-xl border border-slate-200 hover:border-red-100 transition-all active:scale-95 hover:shadow-sm"
            >
              <LogOut size={16} />
              Logout
            </button>
          ...
          {/* Mobile Menu Button */}
          <button
            className="lg:hidden p-2 rounded-xl bg-white shadow-sm border border-slate-200 text-slate-700 active:scale-90 transition-transform"
        ...
        <AnimatePresence>
          {menuOpen && (
            <motion.div 
              ...
              className="lg:hidden bg-white/95 backdrop-blur-xl border-b border-indigo-50 shadow-2xl overflow-hidden"
            >
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 md:px-8 py-8 md:py-12">
        <motion.div 
          ...
          className="bg-white/60 backdrop-blur-2xl border border-white/80 rounded-[2.5rem] shadow-2xl shadow-indigo-100/50 min-h-[600px] overflow-hidden"
        >
          {children}
        </motion.div>
      </main>
      ...
      <footer className="relative z-10 max-w-7xl mx-auto px-6 pb-12">
        <div className="bg-slate-900 rounded-[2rem] p-6 text-white flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl overflow-hidden relative group">
           {/* Glow Effect */}
